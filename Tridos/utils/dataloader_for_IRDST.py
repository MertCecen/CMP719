...
class seqDataset(Dataset):
    def __init__(self, dataset_path, image_size, num_frame=5 ,type='train.txt'):
    ...
    def get_data(self, index):
        ...
        for id in range(0, self.num_frame):
            if image_id - id <= 0:
                ...
                leading_zero_count = 6 - (math.floor(math.log10(image_id-id)) + 1)

            leading_zeros = "0" * leading_zero_count
            img = Image.open(image_path + leading_zeros +'%d.png' % max(image_id - id, 1))
            img = cvtColor(img)
            iw, ih = img.size
